...
class SelfEmbed(nn.Module):
    ...
    def forward(self, data, graph_embedding):
        ...
        return out, y, regularization, stability_regularization

    def embed(self, data, graph_embedding):
        raw_x, edge_idx, sample_id, batch = data.x, data.edge_index, data.sample_id, data.batch
        sample_id =sample_id.T
        sample_id = torch.zeros_like(sample_id) # set one hot to all 0
        sample_id = F.one_hot(sample_id, self.num_sample).float()
        sample_id =  sample_id.to(self.device)

        x, mu = self.Lspin(raw_x, graph_embedding)
        for gcn_block in self.GNN:
            x = gcn_block(x, edge_idx)

        unbatched_out = utils.unbatch(x, batch)
        x = torch.stack([sample[0] for sample in unbatched_out])
        ...
        out_corrected = self.predictor(x)

        cell_idx = [idx[0] for idx in data.cell_idx]
        
        unbatched_x = utils.unbatch(data.x, data.batch)
        x_list = [instance[0,:] for instance in unbatched_x]
        # x is the embedding, x_list is the original input for central node, out_corrected is the prediction when setting sample_id to all 0.
        return x, x_list, data.sample, cell_idx, out_corrected
