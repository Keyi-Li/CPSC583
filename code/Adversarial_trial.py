...
class GCN_adversarial(torch.nn.Module):
    def __init__(self, hidden_channels):
        super(GCN_adversarial, self).__init__()
        ...
        self.feature = nn.Sequential(
            OrderedDict([
                ("f_conv1",GCNConv(33, hidden_channels)),
                ("f_batchnorm1",BatchNorm(hidden_channels)),
                ("f_relu1",nn.ReLU()),
                ("f_dropout1", Dropout(0.25)),
                ("f_conv2",GCNConv(hidden_channels, hidden_channels)),
                ("f_batchnorm2",BatchNorm(hidden_channels)),
                ("f_relu2",nn.ReLU()),
                ("f_dropout2", Dropout(0.25)),
                ("f_conv3",GCNConv(hidden_channels, hidden_channels)),
                ("f_batchnorm3",BatchNorm(hidden_channels)),
                ("f_relu3",nn.ReLU())
            ])
        )
    ...
    def embed(self, data):
        ...
        x, edge_index, batch = data.x, data.edge_index, data.batch
        n_embed = self.feature(x,edge_indx)
        g_embed = global_mean_pool(n_embed, batch)
...
# <img src="./domain_adversarial.png" alt="Model Structure and Training" width="600" height="400">
class ReverseLayerF(Function):
    @staticmethod
...
for (train_idx, test_idx), test_sample_name in zip(fold_splits, combinations[:5]):
    ...
    for epoch in range(n_epoch):
        ...
        for i in range(len_dataloader):
            ...
            class_out, sample_out = my_net(data=data_source, alpha=alpha)
            ...
            total_loss = err_s_main + err_s_domain + err_t_domain
